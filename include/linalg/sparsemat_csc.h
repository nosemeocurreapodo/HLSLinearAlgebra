...
namespace linalg
{
    ...
    public:
        ...
        explicit SparseMatCSC(Index rows, Index cols, bool store_lower_only = true)
            : rows_(rows), cols_(cols), store_lower_only_(store_lower_only)
        {
            // if (n_ < 0) throw std::invalid_argument("SparseMatCSC: n < 0");
        }

        // void resize(Index rows, Index cols, bool store_lower_only = true)
        //{
        ...
        T &insert(Index i, Index j)
        {
            if (rows_ <= 0 || cols_ <= 0)
                throw std::runtime_error("SparseMatCSC::add: matrix size not set");
            if (i < 0 || j < 0 || i >= rows_ || j >= cols_)
                throw std::out_of_range("SparseMatCSC::add: index out of range");

            if (store_lower_only_)
            {
                if (i < j)
                    std::swap(i, j); // fold to lower triangle
            }

            triplets_.push_back({i, j, 0});
            finalized_ = false;

            return triplets_[triplets_.size() - 1].v;
        }
        ...
        {
            if (rows_ <= 0 || cols_ <= 0)
                throw std::runtime_error("SparseMatCSC::finalize: matrix size not set");

            // Build column counts
            std::vector<Index> col_count(cols_, 0);
            ...
            // Prefix sum -> Ap_
            Ap_.assign(cols_ + 1, 0);
            Ap_[0] = 0;
            for (Index c = 0; c < cols_; ++c)
                Ap_[c + 1] = Ap_[c] + col_count[c];

            const Index nnz_raw = Ap_[cols_];
            ...
            std::vector<Index> Ap2(cols_ + 1, 0);
        ...
        const Index *Ap() const
        {
            require_finalized();
            return Ap_.data();
        }
        const Index *Ai() const
        {
            require_finalized();
            return Ai_.data();
        }
    ...
    private:
        ...
        {
            if (!finalized_)
                throw std::runtime_error("SparseMatCSC: call finalize() before accessing CSC arrays");
        }
